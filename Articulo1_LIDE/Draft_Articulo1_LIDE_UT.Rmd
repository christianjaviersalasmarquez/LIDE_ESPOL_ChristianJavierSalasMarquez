...
#df_analisis_UT2019_s51p2 <- data.frame( s51p2a = as.numeric(df_analisis_UT2019$s51p2a), 
#                                        s51p2b = as.numeric(df_analisis_UT2019$s51p2b), 
#                                        s51p2c = as.numeric(df_analisis_UT2019$s51p2c), 
#                                        s51p2d = as.numeric(df_analisis_UT2019$s51p2d) )
...
#df_analisis_UT2019$horas_semanales_cocina <- df_analisis_UT2019_s51p2$s51p2a + 
#                                             (df_analisis_UT2019_s51p2$s51p2b/60) + 
#                                             df_analisis_UT2019_s51p2$s51p2c + 
#                                             (df_analisis_UT2019_s51p2$s51p2d/60)
...
#S51P2_UT2019[,5:8] <-  sapply(S51P2_UT2019[,5:8], FUN = function(x) ifelse(is.na(x), 0, x))
...
#UT15_UT2012[,5:8] <- 
#  sapply(UT15_UT2012[,5:8], FUN = function(x) ifelse(is.na(x), 0, x))
...
#UT15_UT2012 %>% filter(!is.na(UT15)) %>%
#  group_by(sexo) %>%
#  summarize(w_mean = weighted.mean(t_horas_cocina, fexp),
#            w_sd = weighted.sd(t_horas_cocina, fexp),
#            mean = mean(t_horas_cocina),
#            sd = sd(t_horas_cocina),
#            sum = sum(t_horas_cocina), 
#            max = max(t_horas_cocina), 
#            min = min(t_horas_cocina))
...
#df_t_cocina_UT2012_plot2 <- svyby(formula = ~ t_horas_cocina, by = ~ edad_rango,
#      design = UT2012_svydesign,
#      FUN = svymean,
#      na.rm = TRUE, keep.names = FALSE)
...
#df_t_cocina_UT2012_plot3 <- svyby(formula = ~ t_horas_cocina, by = ~ sexo + edad_rango,
#      design = UT2012_svydesign,
#      FUN = svymean,
#      na.rm = TRUE, keep.names = FALSE)
...
#df_t_cocina_UT2019_plot2 <- svyby(formula = ~ t_horas_cocina, by = ~ edad_rango,
#      design = UT2019_svydesign,
#      FUN = svymean,
#      na.rm = TRUE, keep.names = FALSE)
...
Es decir, para el año 2019, observamos un ligero aumento del promedio de horas semanales que las mujeres dedican a cocinar o preparar alimentos y en el caso de los hombres, se observa un aumento del promedio de horas semanales dedicados a la cocina.
...
En el siguiente repositorio de GitHub [GitHub](https://github.com/christianjaviersalasmarquez/LIDE_ESPOL_Christian_Salas/blob/main/Draft_Articulo1_LIDE_UT.Rmd), podrás revisar el código, reproducir el análisis y encontrar cálculos que pueden generar gráficos adicionales.
