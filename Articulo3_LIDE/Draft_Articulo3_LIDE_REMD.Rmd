...
```{r}
modelo2_KM <- survfit(Surv( (df_analisis_EMA_2014 %>% filter(hijos_rec != 99))$duracion_matrimonio , (df_analisis_EMA_2014 %>% filter(hijos_rec != 99))$evento_divorcio ) ~ (df_analisis_EMA_2014 %>% filter(hijos_rec != 99))$hijos_rec )
modelo2_KM
```
...
```{r}
autoplot(modelo2_KM, conf.int = FALSE) + ylab('S(t)') + ggtitle('Curva de Supervivencia de KM Estimada')
```
...
```{r}
autoplot(modelo3_KM, conf.int = FALSE) + ylab('S(t)') + ggtitle('Curva de Supervivencia de KM Estimada')
```
...
```{r}
R_divorcios_2014 <- dim(df_analisis_EMA_2014)[1]
nacionalidad_pareja <- rep(0, R_divorcios_2014)
...
```{r}
autoplot(modelo4_KM, conf.int = FALSE) + ylab('S(t)') + ggtitle('Curva de Supervivencia de KM Estimada')
```
