...
#MEJORAR, ESPECIFICA SOLO A LAS COLUMNAS REQUERIDAS
...
SOLO ASEGURAR SI LOS NA CON 0 ERAN DE PERSONAS QUE EN REALIDAD TUVIERON 0 HORAS EN LA COCINA

REVISAR SI EN LA ENCUESTA HAY NAS EN LA PREGUNTA DE QUEHACERES EN EL HOGAR

```{r}
sum(is.na(data_UT2019$s51p1))
```
...
EL NUMERO DE PERSONAS NO ES TAN INFORMATIVO PORQUE ES UNA MUESTRA. QUEREMOS CALCULAR EL PARAMETRO DE LA MEDIA. ENTONCES LA MEDIA. EN EJE DE LAS X CAMBIAR A PORCENTAJE.
...
# Se considera los que responden que NO a s51p1 '¿Participa en los quehaceres del hogar... ?, pues se considera que la semana pasada aporto 0 horas a la cocina. Se considera a los que responden que SI cocinó y a los que responden que NO porque de igual forma se considera que la semana pasada aporto 0 horas a la cocina.

df_analisis_UT2019 %>%
  filter( s51p1_factor == "No" | s51p2_factor == "Si" | s51p2_factor == "No" ) %>%
  group_by(s1p2_factor) %>%
  summarize(promedio_horas_cocina = mean(horas_semanales_cocina),media_ponderada_horas_cocina = weighted.mean(horas_semanales_cocina, fexp_UT2019), sd_horas_cocina = sd(horas_semanales_cocina), suma = sum(horas_semanales_cocina), max = max(horas_semanales_cocina), min = min(horas_semanales_cocina))
```
...
```{r}
# NO ES NECESARIA LA REDUNDANCIA
...
```{r}
# Promedio de horas semanales en cocinar o preparar alimentos en 2019 por sexo. METODO DE ALONSO (Sin haber especifcado NA en s51p2_factor)
df_analisis_UT2019 %>%
  filter( !is.na(s51p2_factor) ) %>%
  group_by(s1p2_factor) %>%
  summarize(promedio_horas_cocina = mean(horas_semanales_cocina), media_ponderada_horas_cocina = weighted.mean(horas_semanales_cocina, fexp_UT2019), sd_horas_cocina = sd(horas_semanales_cocina), suma = sum(horas_semanales_cocina), max = max(horas_semanales_cocina), min = min(horas_semanales_cocina))
```
...
```{r}
df_analisis_UT2019$s51p2_factor_TEST <- factor(df_analisis_UT2019$s51p2)
levels(df_analisis_UT2019$s51p2_factor_TEST) <- c(NA,'Si','No')


# Promedio de horas semanales en cocinar o preparar alimentos en 2019 por sexo. METODO DE ALONSO (Habiendo especificado NA en s51p2_factor_TEST)
df_analisis_UT2019 %>%
  filter( !is.na(s51p2_factor_TEST) ) %>%
  group_by(s1p2_factor) %>%
  summarize(promedio_horas_cocina = mean(horas_semanales_cocina), media_ponderada_horas_cocina = weighted.mean(horas_semanales_cocina, fexp_UT2019), sd_horas_cocina = sd(horas_semanales_cocina), suma = sum(horas_semanales_cocina), max = max(horas_semanales_cocina), min = min(horas_semanales_cocina))
```
...
```{r}
# VALIDACION COMPARANDO METODOS
df_analisis_UT2019 %>%
   mutate( Alonso = !is.na(s51p2_factor), Alonso2 =  !is.na(s51p2_factor_TEST) , Christian = (s51p2_factor == "Si" | s51p2_factor == "No"), Christian2 = ( s51p1_factor == "No" | s51p2_factor == "Si" | s51p2_factor == "No" ) ) %>%
  select(s1p3,s51p1, s51p1_factor,s51p2,s51p2_factor, s51p2_factor_TEST,Alonso, Alonso2, Christian, Christian2, horas_semanales_cocina) %>%
  view()
```
