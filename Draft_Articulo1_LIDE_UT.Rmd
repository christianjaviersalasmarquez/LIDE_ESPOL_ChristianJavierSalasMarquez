---
title: "Uso del tiempo de los ecuatorianos 1"
author: "Christian Javier Salas Marquez"
date: '2022-03-17'
output:
  html_document:
    df_print: paged
---

Work in progress :) 

Last edited: 31-03-2022

```{r include=FALSE}
###############################################################
####                   Librerias                           ####
###############################################################

if(!require(tidyverse)) install.packages("tidyverse", repos = "http://cran.us.r-project.org")
if(!require(haven)) install.packages("haven", repos = "http://cran.us.r-project.org")
if(!require(readr)) install.packages("readr", repos = "http://cran.us.r-project.org")
if(!require(gridExtra)) install.packages("gridExtra", repos = "http://cran.us.r-project.org")

if(!require(foreign)) install.packages("foreign", repos = "http://cran.us.r-project.org")
if(!require(survey)) install.packages("survey", repos = "http://cran.us.r-project.org")
if(!require(srvyr)) install.packages("srvyr", repos = "http://cran.us.r-project.org")


```

```{r include=FALSE}
###############################################################
####                   Descargar los datos                 ####
###############################################################


# Base de datos 
# Le damos un nombre al url
url <- "https://github.com/christianjaviersalasmarquez/LIDE_ESPOL_Christian_Salas/raw/main/Base%20EUT%202012.zip"
# Creamos un directorio temporal
td <- tempdir()
# Creamos una carpeta temporal
tf <- tempfile(tmpdir=td, fileext = ".zip")
# Descargamos los discap en la carpeta temporal
download.file(url,tf)

```

```{r include=FALSE}
# Obtenemos el nombre del archivo dentro del archivo zip, lo descomprimimos (unzip), obtenemos el nombre del 
# parche, y finalmente lo cargamos
EUT2012.f.name <- unzip(tf, list=TRUE)$Name[1] # El archivo SAV de la EUT 2012 es el primero
unzip(tf, files=EUT2012.f.name, exdir=td, overwrite=TRUE)
EUT2012.f.path <- file.path(td, EUT2012.f.name)
uso_del_tiempo_2012 <- read_sav(EUT2012.f.path)
```

```{r include=FALSE}
data_UT2012 <- data.frame(uso_del_tiempo_2012)
```

```{r include=FALSE}
uso_del_tiempo_2019 <- read.csv2("https://raw.githubusercontent.com/christianjaviersalasmarquez/LIDE_ESPOL_Christian_Salas/main/201912_multibdd_uso_del_tiempo.sav.csv", header = FALSE)

personas_2019 <- read.csv2("https://raw.githubusercontent.com/christianjaviersalasmarquez/LIDE_ESPOL_Christian_Salas/main/201912_multibdd_personas.sav.csv", header = FALSE)
```




```{r include=FALSE}
header_true <- function(df) {
  names(df) <- as.character(unlist(df[1,]))
  df[-1,]
}
```

```{r include=FALSE}
data_UT2019 <- header_true(uso_del_tiempo_2019)

colnames(data_UT2019)[1] <- 'area'
```

```{r include=FALSE}
data_personas2019 <- header_true(personas_2019)

colnames(data_personas2019)[1] <- 'area'
```



En Ecuador y gran parte del mundo es bien sabido que las mujeres dedican mucho más tiempo a la cocina que los hombres (CITAR). Esto es una realidad inclusive cuando ambos trabajan (CITAR). En nuestro país, utilizando las bases de datos del INEC, podemos contabilizar el tiempo que los hombres y las mujeres dedican a diversas actividades, entre esas el tiempo dedicado a cocinar o preparar alimentos. En el mundo, cada vez más hombres, inclusive cuanto trabajan, toman más el mando de las tareas del hogar. En este artículo, voy a comparar cómo ha cambiado el tiempo dedicado a la cocina desde el 2012 al 2019 (tiempo de prepandemia).

El objetivo del siguiente artículo es realizar una comparación del promedio de horas semanales dedicadas a cocinar o preparar alimentos por diferentes categorías de interés nacional entre los años 2012 y 2019. Queremos identificar la evolución de esta medida entre los dos años mencionados. 

De acuerdo con el INEC, las encuestas de uso del tiempo son importantes porque proporcionan una forma de conocer la distribución de tiempos entre hombres y mujeres. Además, es importante considerar el promedio de horas semanales dedicado a la cocina dado que el trabajo doméstico y las horas dedicadas a la cocina, se consideran como trabajo no remunerado. El trabajo no remunerado es una de las métricas estudiadas por el Instituto Nacional de Estadística y Censos. Según el INEC, los ecuatorianos generan aproximadamente 12 mil millones de horas de trabajo no remunerado en los hogares. En particular, el 76.8% de las horas de trabajo no remunerado fueron efectuadas por mujeres, en comparación con el 23.2% de los hombres. 

Según el RIMISP en su artículo del 6 de marzo del 2020, las mujeres dedican 3 veces más tiempo al trabajo no remunerado. En partícular, según el INEC, en el Ecuador, las mujeres dedican 31 horas semanales al trabajo doméstico, mientras que los hombres solo 11.3 horas. Queremos investigar en particular el tiempo dedicado a la cocina, dado que es parte del agregado del trabajo doméstico.

Para ejecutar este análisis, usé las encuestas del uso del tiempo del 2012 y 2019 que levantó el INEC. En estos levantamientos, se hicieron preguntas a los ecuatorianos para poder identificar la cantidad de horas semanales dedicadas a alguna actividad en específica al entrevistado. Ambas encuestas son representativas a nivel nacional. En este artículo, realicé los cálculos de las medias ponderadas de horas semanales dedicado a cocinar o preparar alimentos separando por las variables categóricas de sexo, provincia y grupo de edad.


METODOLOGIA. (Colocar al final)

Para contestar este análisis, usé las encuestas de uso del tiempo. La primera fue levantada en el año 2012, se ejecutó la Encuesta Específica de Uso del Tiempo, la cual tiene un tamaño muestral de 22968 viviendas, cuya población objetivo son las personas de 12 años y más. El periodo de levantamiento fue entre noviembre y diciembre de 2012. La segunda fue levantada en el año 2019, el uso del tiempo es un módulo de la Encuesta Nacional Multipropósito de Hogares. Entre las diferencias metodológicas, en la Encuesta Nacional Multipropósito de Hogares la población objetivo abarca los hogares y personas constituidas por personas de 5 años y más. El tamaño muestral fue de 12072 viviendas.  MODIFICAR.





```{r include=FALSE}
###############################################################
####                        2012                           ####
###############################################################
# UT15: EN LA SEMANA PASADA... ¿Cocinó o preparó alimentos para consumirse en el desayuno, almuerzo, merienda, entre comidas, incluyendo comidas para llevar?

df_analisis_UT15_UT2012 <- data.frame(P02_factor = fct_recode(factor(data_UT2012$P02), "Hombre" = "1", "Mujer"="2") , CIUDAD = data_UT2012$CIUDAD, UT15 = data_UT2012$UT15 , UT15_factor = fct_recode(factor(data_UT2012$UT15), "Si"="1","No"="2" ), P03 = data_UT2012$P03 , UT15A = data_UT2012$UT15A, UT15B = data_UT2012$UT15B, UT15C = data_UT2012$UT15C, UT15D = data_UT2012$UT15D, fexp_UT2012 = data_UT2012$fexp )

```

```{r include=FALSE}
# Rango de edades 2012

edad_2012 <- as.numeric(df_analisis_UT15_UT2012$P03)

# 98: 98 y más
# 99: No informa

R_edad_2012 = length(edad_2012)
rango_edad_10_UT2012 <- rep(0,R_edad_2012)

for (i in 1:R_edad_2012) {
  
  if((edad_2012[i] >= 0 ) & (edad_2012[i] <= 11)) { rango_edad_10_UT2012[i] <- 'Edad entre 0 y 11 años' }
  else if ( (edad_2012[i] >=12  ) & (edad_2012[i] <= 19  ) ) { rango_edad_10_UT2012[i] <- 'Edad entre 12 y 19 años' }
  else if ( (edad_2012[i] >=20  ) & (edad_2012[i] <= 29  ) ) { rango_edad_10_UT2012[i] <- 'Edad entre 20 y 29 años' }
  else if ( (edad_2012[i] >=30  ) & (edad_2012[i] <= 39  ) ) { rango_edad_10_UT2012[i] <- 'Edad entre 30 y 39 años' }
  else if ( (edad_2012[i] >=40  ) & (edad_2012[i] <= 49  ) ) { rango_edad_10_UT2012[i] <- 'Edad entre 40 y 49 años' }
  else if ( (edad_2012[i] >=50  ) & (edad_2012[i] <= 59  ) ) { rango_edad_10_UT2012[i] <- 'Edad entre 50 y 59 años' }
  else if ( (edad_2012[i] >=60  ) & (edad_2012[i] <= 69  ) ) { rango_edad_10_UT2012[i] <- 'Edad entre 60 y 69 años' }
  else if ( (edad_2012[i] >=70  ) & (edad_2012[i] <= 98  )) { rango_edad_10_UT2012[i] <- 'Edad mayor a 70 años' }
  else if ( edad_2012[i] == 99  ) { rango_edad_10_UT2012[i] <- 'No informa' }
  
  
}

df_analisis_UT15_UT2012$rango_edad_10_UT2012 <- factor(rango_edad_10_UT2012)
```

```{r include=FALSE}
# VALIDADO 100%
# Segmentador de provincias
# En la variable ciudad, el INEC maneja un código de 6 dígitos, donde el 1er y 2do dígito forman el Código de Provincia, el 3er y 4to dígito forman el Código de Cantón y el 5to y 6to dígito forman el Código de Parroquia. Por ejemplo, en el código 010150, 01 indica la provincia de Azuay, 0101 indica el cantón Cuenca y 010150 indica la parroquia 'Cuenca'
#No existen registros para Galápagos en el dataset del 2012

codigo_ciudad_2012 <- as.character(df_analisis_UT15_UT2012$CIUDAD)


R_provincia_2012 <- length(codigo_ciudad_2012)
provincia_UT2012 <- rep(0, R_provincia_2012)

for (i in 1:R_provincia_2012) {
  if (nchar(codigo_ciudad_2012[i]) == 5 ) {
    codigo_ciudad_2012[i] <- paste("0",codigo_ciudad_2012[i], sep = "")
  }
}




for (i in 1:R_provincia_2012) {
  
  if (substr(codigo_ciudad_2012[i], start = 1, stop = 2) == "01") { provincia_UT2012[i] <- 'Azuay' }
  else if (substr(codigo_ciudad_2012[i], start = 1, stop = 2) == "02") { provincia_UT2012[i] <- 'Bolívar' }
  else if (substr(codigo_ciudad_2012[i], start = 1, stop = 2) == "03") { provincia_UT2012[i] <- 'Cañar' }
  else if (substr(codigo_ciudad_2012[i], start = 1, stop = 2) == "04") { provincia_UT2012[i] <- 'Carchi' }
  else if (substr(codigo_ciudad_2012[i], start = 1, stop = 2) == "05") { provincia_UT2012[i] <- 'Cotopaxi' }
  else if (substr(codigo_ciudad_2012[i], start = 1, stop = 2) == "06") { provincia_UT2012[i] <- 'Chimborazo' }
  else if (substr(codigo_ciudad_2012[i], start = 1, stop = 2) == "07") { provincia_UT2012[i] <- 'El Oro' }
  else if (substr(codigo_ciudad_2012[i], start = 1, stop = 2) == "08") { provincia_UT2012[i] <- 'Esmeraldas' }
  else if (substr(codigo_ciudad_2012[i], start = 1, stop = 2) == "09") { provincia_UT2012[i] <- 'Guayas' }
  else if (substr(codigo_ciudad_2012[i], start = 1, stop = 2) == "10") { provincia_UT2012[i] <- 'Imbabura' }
  else if (substr(codigo_ciudad_2012[i], start = 1, stop = 2) == "11") { provincia_UT2012[i] <- 'Loja' }
  else if (substr(codigo_ciudad_2012[i], start = 1, stop = 2) == "12") { provincia_UT2012[i] <- 'Los Ríos' }
  else if (substr(codigo_ciudad_2012[i], start = 1, stop = 2) == "13") { provincia_UT2012[i] <- 'Manabí' }
  else if (substr(codigo_ciudad_2012[i], start = 1, stop = 2) == "14") { provincia_UT2012[i] <- 'Morona Santiago' }
  else if (substr(codigo_ciudad_2012[i], start = 1, stop = 2) == "15") { provincia_UT2012[i] <- 'Napo' }
  else if (substr(codigo_ciudad_2012[i], start = 1, stop = 2) == "16") { provincia_UT2012[i] <- 'Pastaza' }
  else if (substr(codigo_ciudad_2012[i], start = 1, stop = 2) == "17") { provincia_UT2012[i] <- 'Pichincha' }
  else if (substr(codigo_ciudad_2012[i], start = 1, stop = 2) == "18") { provincia_UT2012[i] <- 'Tungurahua' }
  else if (substr(codigo_ciudad_2012[i], start = 1, stop = 2) == "19") { provincia_UT2012[i] <- 'Zamora Chinchipe' }
  else if (substr(codigo_ciudad_2012[i], start = 1, stop = 2) == "20") { provincia_UT2012[i] <- 'Galápagos' } 
  else if (substr(codigo_ciudad_2012[i], start = 1, stop = 2) == "21") { provincia_UT2012[i] <- 'Sucumbíos' }
  else if (substr(codigo_ciudad_2012[i], start = 1, stop = 2) == "22") { provincia_UT2012[i] <- 'Orellana' }
  else if (substr(codigo_ciudad_2012[i], start = 1, stop = 2) == "23") { provincia_UT2012[i] <- 'Santo Domingo de los Tsáchilas' }
  else if (substr(codigo_ciudad_2012[i], start = 1, stop = 2) == "24") { provincia_UT2012[i] <- 'Santa Elena' }
  else if (substr(codigo_ciudad_2012[i], start = 1, stop = 2) == "90") { provincia_UT2012[i] <- 'Zonas no delimitadas' }
  
}

df_analisis_UT15_UT2012$provincia_UT2012 <- factor(provincia_UT2012)
```





```{r include=FALSE}
###############################################################
####                Cálculos UT15 2012                     ####
###############################################################
# df_UT15_ABCD_UT2012 es un data frame auxiliar para calcular la cantidad horas_semanales_cocina

df_UT15_ABCD_UT2012 <- data.frame( UT15A = data_UT2012$UT15A, UT15B = data_UT2012$UT15B, UT15C = data_UT2012$UT15C, UT15D = data_UT2012$UT15D  )

df_UT15_ABCD_UT2012[is.na(df_UT15_ABCD_UT2012)] <- 0

df_analisis_UT15_UT2012$horas_semanales_cocina <- df_UT15_ABCD_UT2012$UT15A + (df_UT15_ABCD_UT2012$UT15B/60) + df_UT15_ABCD_UT2012$UT15C + (df_UT15_ABCD_UT2012$UT15D/60)

```

```{r include=FALSE}
# Promedio de horas semanales en cocinar o preparar alimentos de los ecuatorianos en el 2012
# No se consideran los NA values porque la pregunta UT15 está dirigida a informantes de 12 en adelante. Los NA values están conformados por los informantes que tienen menos de 12.

df_t_cocina_UT2012_plot1 <-  df_analisis_UT15_UT2012 %>%
  filter( UT15_factor == "Si" | UT15_factor == "No" ) %>%
  group_by(P02_factor) %>%
  summarize(promedio_horas_cocina = mean(horas_semanales_cocina), media_ponderada_horas_cocina = weighted.mean(horas_semanales_cocina, fexp_UT2012) , sd_horas_cocina = sd(horas_semanales_cocina), suma = sum(horas_semanales_cocina), max = max(horas_semanales_cocina), min = min(horas_semanales_cocina))

df_t_cocina_UT2012_plot1

```

```{r include=FALSE}
df_t_cocina_UT2012_plot2 <-  df_analisis_UT15_UT2012 %>%
  filter( UT15_factor == "Si" | UT15_factor == "No" ) %>%
  group_by(rango_edad_10_UT2012) %>%
  summarize(promedio_horas_cocina = mean(horas_semanales_cocina), media_ponderada_horas_cocina = weighted.mean(horas_semanales_cocina, fexp_UT2012) , sd_horas_cocina = sd(horas_semanales_cocina), suma = sum(horas_semanales_cocina), max = max(horas_semanales_cocina), min = min(horas_semanales_cocina))

df_t_cocina_UT2012_plot2

```

```{r include=FALSE}
df_t_cocina_UT2012_plot3 <- df_analisis_UT15_UT2012 %>%
  filter( UT15_factor == "Si" | UT15_factor == "No" ) %>%
  group_by(rango_edad_10_UT2012, P02_factor) %>%
  summarize(promedio_horas_cocina = mean(horas_semanales_cocina), media_ponderada_horas_cocina = weighted.mean(horas_semanales_cocina, fexp_UT2012) , sd_horas_cocina = sd(horas_semanales_cocina), suma = sum(horas_semanales_cocina), max = max(horas_semanales_cocina), min = min(horas_semanales_cocina))

df_t_cocina_UT2012_plot3

```

```{r include=FALSE}
df_t_cocina_UT2012_plot4 <- df_analisis_UT15_UT2012 %>%
  filter( UT15_factor == "Si" | UT15_factor == "No" ) %>%
  group_by(provincia_UT2012) %>%
  summarize(promedio_horas_cocina = mean(horas_semanales_cocina), media_ponderada_horas_cocina = weighted.mean(horas_semanales_cocina, fexp_UT2012) , sd_horas_cocina = sd(horas_semanales_cocina), suma = sum(horas_semanales_cocina), max = max(horas_semanales_cocina), min = min(horas_semanales_cocina))

df_t_cocina_UT2012_plot4

```

```{r include=FALSE}
df_t_cocina_UT2012_plot5 <-  df_analisis_UT15_UT2012 %>%
  filter( UT15_factor == "Si" | UT15_factor == "No" ) %>%
  group_by(provincia_UT2012, P02_factor) %>%
  summarize(promedio_horas_cocina = mean(horas_semanales_cocina), media_ponderada_horas_cocina = weighted.mean(horas_semanales_cocina, fexp_UT2012) , sd_horas_cocina = sd(horas_semanales_cocina), suma = sum(horas_semanales_cocina), max = max(horas_semanales_cocina), min = min(horas_semanales_cocina))

df_t_cocina_UT2012_plot5

```











```{r include=FALSE}
###############################################################
####                        2019                           ####
###############################################################

#Merge de la base de datos de uso del tiempo con la de personas, para conocer información sobre la persona

df_info_personas2019 <- data.frame(id_per = data_personas2019$id_per, s1p2 = data_personas2019$s1p2, s1p3 = data_personas2019$s1p3)

df_analisis_UT2019 <- merge(df_info_personas2019, data_UT2019, by = "id_per")

```

```{r include=FALSE}
# df_analisis_UT2019_s51p2 es un data frame auxiliar para calcular la cantidad horas_semanales_cocina

df_analisis_UT2019_s51p2 <- data.frame( s51p2a = as.numeric(df_analisis_UT2019$s51p2a), s51p2b = as.numeric(df_analisis_UT2019$s51p2b), s51p2c = as.numeric(df_analisis_UT2019$s51p2c), s51p2d = as.numeric(df_analisis_UT2019$s51p2d) )

# LOS NA PERTENECEN A LAS PERSONAS DE PERSONAS QUE REPORTARON NO HABER REALIZADO TAREAS EN EL HOGAR. POR LO TANTO SE REEMPLAZAN LOS NAs CON 0.

df_analisis_UT2019_s51p2[is.na(df_analisis_UT2019_s51p2)] <- 0 

df_analisis_UT2019$horas_semanales_cocina <- df_analisis_UT2019_s51p2$s51p2a + (df_analisis_UT2019_s51p2$s51p2b/60) + df_analisis_UT2019_s51p2$s51p2c + (df_analisis_UT2019_s51p2$s51p2d/60)

```

```{r include=FALSE}
df_analisis_UT2019$s1p2_factor <- fct_recode(factor(df_analisis_UT2019$s1p2), "Hombre"="1","Mujer"="2" )
df_analisis_UT2019$s51p1_factor <- fct_recode(factor(df_analisis_UT2019$s51p1), "Si"="1","No"="2" )
df_analisis_UT2019$s51p2_factor <- fct_recode(factor(df_analisis_UT2019$s51p2), "Si"="1","No"="2" ) 
```

```{r include=FALSE}
# En el dataset, fexp está compuesta por valores del tipo character con coma, por lo cual R no puede transformar a numeric sin que aparezca el warning NAs introduced by coercion transformando todos los valores en NA. Primero es necesario reemplazar las comas por puntos

R_fexp <- length(df_analisis_UT2019$fexp)
fexp_UT2019 <- rep(0,R_fexp)

for (i in 1:R_fexp) {
  fexp_UT2019[i] <- gsub(",",".", df_analisis_UT2019$fexp[i])
  
}

df_analisis_UT2019$fexp_UT2019 <- as.numeric(fexp_UT2019)

```

```{r include=FALSE}
df_analisis_UT2019$upm_factor <- factor(df_analisis_UT2019$upm)
df_analisis_UT2019$estrato_factor <- factor(df_analisis_UT2019$estrato)
```

```{r include=FALSE}
# Rango de edades 2019

edad_2019 <- as.numeric(df_analisis_UT2019$s1p3)

R_edad_2019 = length(edad_2019)
rango_edad_10_UT2019 <- rep(0,R_edad_2019)

for (i in 1:R_edad_2019) {
  
  if((edad_2019[i] >= 0 ) & (edad_2019[i] <= 11)) { rango_edad_10_UT2019[i] <- 'Edad entre 0 y 11 años' }
  else if ( (edad_2019[i] >=12  ) & (edad_2019[i] <= 19  ) ) { rango_edad_10_UT2019[i] <- 'Edad entre 12 y 19 años' }
  else if ( (edad_2019[i] >=20  ) & (edad_2019[i] <= 29  ) ) { rango_edad_10_UT2019[i] <- 'Edad entre 20 y 29 años' }
  else if ( (edad_2019[i] >=30  ) & (edad_2019[i] <= 39  ) ) { rango_edad_10_UT2019[i] <- 'Edad entre 30 y 39 años' }
  else if ( (edad_2019[i] >=40  ) & (edad_2019[i] <= 49  ) ) { rango_edad_10_UT2019[i] <- 'Edad entre 40 y 49 años' }
  else if ( (edad_2019[i] >=50  ) & (edad_2019[i] <= 59  ) ) { rango_edad_10_UT2019[i] <- 'Edad entre 50 y 59 años' }
  else if ( (edad_2019[i] >=60  ) & (edad_2019[i] <= 69  ) ) { rango_edad_10_UT2019[i] <- 'Edad entre 60 y 69 años' }
  else if ( (edad_2019[i] >=70  )) { rango_edad_10_UT2019[i] <- 'Edad mayor a 70 años' }
  
  
}

df_analisis_UT2019$rango_edad_10_UT2019 <- factor(rango_edad_10_UT2019)
```

```{r include=FALSE}
# Creación de variable de provincia

df_analisis_UT2019$provincia_UT2019 <- substr(df_analisis_UT2019$ciudad,start = 1, stop = 2)

df_analisis_UT2019$provincia_UT2019 <- as.factor(df_analisis_UT2019$provincia_UT2019)

levels(df_analisis_UT2019$provincia_UT2019) <- c('Azuay','Bolívar','Cañar','Carchi','Cotopaxi','Chimborazo','El Oro','Esmeraldas','Guayas','Imbabura','Loja','Los Ríos','Manabí','Morona Santiago','Napo','Pastaza','Pichincha','Tungurahua','Zamora Chinchipe','Galápagos','Sucumbíos','Orellana','Santo Domingo de los Tsáchilas','Santa Elena','Zonas no delimitadas')

```





```{r include=FALSE}
###############################################################
####                Cálculos s51p2 2019                    ####
###############################################################
# Promedio de horas semanales en cocinar o preparar alimentos en 2019 por sexo. CONSIDERANDO FACTORES DE EXPANSION.

df_t_cocina_UT2019_plot1 <- df_analisis_UT2019 %>%
  group_by(s1p2_factor) %>%
  summarize(promedio_horas_cocina = mean(horas_semanales_cocina),media_ponderada_horas_cocina = weighted.mean(horas_semanales_cocina, fexp_UT2019), sd_horas_cocina = sd(horas_semanales_cocina), suma = sum(horas_semanales_cocina), max = max(horas_semanales_cocina), min = min(horas_semanales_cocina))

df_t_cocina_UT2019_plot1


```

```{r include=FALSE}
df_t_cocina_UT2019_plot2 <- df_analisis_UT2019 %>%
  group_by(rango_edad_10_UT2019) %>%
  summarize(promedio_horas_cocina = mean(horas_semanales_cocina),media_ponderada_horas_cocina = weighted.mean(horas_semanales_cocina, fexp_UT2019), sd_horas_cocina = sd(horas_semanales_cocina), suma = sum(horas_semanales_cocina), max = max(horas_semanales_cocina), min = min(horas_semanales_cocina))

df_t_cocina_UT2019_plot2

```

```{r include=FALSE}
df_t_cocina_UT2019_plot3 <-  df_analisis_UT2019 %>%
  group_by(rango_edad_10_UT2019, s1p2_factor) %>%
  summarize(promedio_horas_cocina = mean(horas_semanales_cocina),media_ponderada_horas_cocina = weighted.mean(horas_semanales_cocina, fexp_UT2019), sd_horas_cocina = sd(horas_semanales_cocina), suma = sum(horas_semanales_cocina), max = max(horas_semanales_cocina), min = min(horas_semanales_cocina))

df_t_cocina_UT2019_plot3

```

```{r include=FALSE}
df_t_cocina_UT2019_plot4 <-  df_analisis_UT2019 %>%
  group_by(provincia_UT2019) %>%
  summarize(promedio_horas_cocina = mean(horas_semanales_cocina),media_ponderada_horas_cocina = weighted.mean(horas_semanales_cocina, fexp_UT2019), sd_horas_cocina = sd(horas_semanales_cocina), suma = sum(horas_semanales_cocina), max = max(horas_semanales_cocina), min = min(horas_semanales_cocina))

df_t_cocina_UT2019_plot4

```

```{r include=FALSE}
df_t_cocina_UT2019_plot5 <-  df_analisis_UT2019 %>%
  group_by(provincia_UT2019, s1p2_factor) %>%
  summarize(promedio_horas_cocina = mean(horas_semanales_cocina),media_ponderada_horas_cocina = weighted.mean(horas_semanales_cocina, fexp_UT2019), sd_horas_cocina = sd(horas_semanales_cocina), suma = sum(horas_semanales_cocina), max = max(horas_semanales_cocina), min = min(horas_semanales_cocina))

df_t_cocina_UT2019_plot5

```











```{r echo=FALSE, fig.height=6 , fig.width=12}
###############################################################
####                Gráficos 2012 vs 2019                  ####
###############################################################

grid.arrange(

ggplot( df_t_cocina_UT2012_plot1 , aes(x = P02_factor , y = media_ponderada_horas_cocina, fill = P02_factor )) + 
  geom_bar(stat = 'identity', position = 'dodge') + 
  scale_fill_manual(values =c("#5B7DB1","#FFAC8E")) +
  ylim(0,8.5) +
  labs(title = "2012", caption = " ", fill = "Sexo") +
  geom_text(aes(label = round(media_ponderada_horas_cocina, digits = 2)  ), color ="black", size = 4,position = position_dodge(0.9),vjust = -0.2) +
  xlab("Sexo") +
  ylab("Media ponderada de horas") +
  theme_classic()
,
ggplot( df_t_cocina_UT2019_plot1 , aes(x = s1p2_factor , y = media_ponderada_horas_cocina, fill = s1p2_factor )) + 
  geom_bar(stat = 'identity', position = 'dodge') + 
  scale_fill_manual(values =c("#5B7DB1","#FFAC8E")) +
  #scale_fill_hue(direction = -1) +
  ylim(0,8.5) +
  labs(title = "2019", caption = " ", fill = "Sexo") +
  geom_text(aes(label = round(media_ponderada_horas_cocina, digits = 2)  ), color ="black", size = 4,position = position_dodge(0.9),vjust = -0.2) +
  xlab("Sexo") +
  ylab("Media ponderada de horas") +
  theme_classic()
,
ncol=2
,
top = textGrob("Tiempo semanal en cocinar o preparar alimentos, por sexo 2012 vs 2019 (horas)", gp=gpar(fontsize=14,font=1)))

```

En esta primera visualización, podemos ver dos gráficos que muestran el tiempo semanal en cocinar o preparar alimentos por sexo. El gráfico de la izquierda es para el año 2012 y el de la derecha para el año 2019. Para el resto de visualizaciones de este artículo se cumple el mismo orden.

En el 2012, se puede observar que, en promedio, las mujeres dedicaron 8.34 horas semanales en cocinar o preparar alimentos, mientras que los hombres solo dedicaron 0.88 horas semanales. Por otro lado, en el 2019, podemos observar que, en promedio, las mujeres dedicaron 6.43 horas semanales en cocinar o preparar alimentos, mientras que los hombres dedicaron esta vez 1.59 horas semanales. 

Es decir, para el año 2019, se observa una reducción del promedio de horas semanales que las mujeres dedican a cocinar o preparar alimentos. En el caso de los hombres, se observa un aumento del promedio de horas semanales dedicados a la cocina. Podrían existir diversos factores que expliquen este cambio en el promedio.



```{r echo=FALSE, fig.height=6 , fig.width=12}

grid.arrange(

ggplot( df_t_cocina_UT2012_plot2 , aes(x = media_ponderada_horas_cocina , y = rango_edad_10_UT2012 )) + 
  geom_bar(stat = 'identity', position = 'dodge', fill = "lightblue") + 
  labs(title = "2012", caption = "") +
  geom_text(aes(label = round(media_ponderada_horas_cocina, digits = 2)  ), color ="#1A374D", size = 4,position = position_dodge(0.9),hjust = 1.2) +
  xlab("Promedio de horas") +
  ylab("Grupo de edad") +
  theme_classic()
,
ggplot( df_t_cocina_UT2019_plot2 , aes(x = media_ponderada_horas_cocina , y = rango_edad_10_UT2019 )) + 
  geom_bar(stat = 'identity', position = 'dodge', fill = "lightblue") + 
  labs(title = "2019", caption = "") +
  geom_text(aes(label = round(media_ponderada_horas_cocina, digits = 2)  ), color ="#1A374D", size = 4,position = position_dodge(0.9),hjust = 1.2) +
  xlab("Promedio de horas") +
  ylab("Grupo de edad") +
  theme_classic()

,
ncol=2
,
top = textGrob("Tiempo semanal en cocinar o preparar alimentos, por grupo de edad 2012 vs 2019 (horas)", gp=gpar(fontsize=14,font=1)))

```


Para el 2012, considerando los grupos de edades, podemos ver que el grupo de edad entre 50 y 59 años es el que acumula la mayor cantidad de horas promedio semanales en cocinar o preparar alimentos con 6.655 horas. Le sigue el grupo de personas que tienen 30 y 39 años.

Para el 2012, considerando los grupos de edades, podemos ver que el grupo de edad entre 50 y 59 años es el que acumula la mayor cantidad de horas promedio semanales en cocinar o preparar alimentos con 5.42 horas. Le sigue el grupo de personas que tienen 40 y 49 años con 5.37 horas.


```{r echo=FALSE, fig.height=6 , fig.width=12}

grid.arrange(

ggplot( df_t_cocina_UT2012_plot3 , aes(x = media_ponderada_horas_cocina , y = rango_edad_10_UT2012 , fill = P02_factor )) + 
  geom_bar(stat = 'identity', position = 'dodge') + 
  scale_fill_manual(values =c("#647A8F","#FFAC8E")) +
  labs(title = "2012", caption = "", fill = "Sexo") +
  geom_text(aes(label = round(media_ponderada_horas_cocina, digits = 2)  ), color ="black", size = 4,position = position_dodge(0.9), hjust = 0.4) +
  xlab("Media ponderada de horas") +
  ylab("Grupo de edad") +
  theme_classic()
,
ggplot( df_t_cocina_UT2019_plot3 , aes(x = media_ponderada_horas_cocina , y = rango_edad_10_UT2019 , fill = s1p2_factor )) + 
  geom_bar(stat = 'identity', position = 'dodge') + 
  scale_fill_manual(values =c("#5B7DB1","#FFAC8E")) +
  labs(title = "2019", caption = "", fill = "Sexo") +
  geom_text(aes(label = round(media_ponderada_horas_cocina, digits = 2)  ), color ="black", size = 4,position = position_dodge(0.9), hjust = 0) +
  xlab("Media ponderada de horas") +
  ylab("Grupo de edad") +
  theme_classic()

,
ncol=2
,
top = textGrob("Tiempo semanal en cocinar o preparar alimentos, por grupo de edad, por sexo 2012 vs 2019 (horas)", gp=gpar(fontsize=14,font=1)))

```





```{r echo=FALSE, fig.height=6 , fig.width=12}

grid.arrange(

ggplot( df_t_cocina_UT2012_plot3 , aes(x = media_ponderada_horas_cocina , y = rango_edad_10_UT2012 , fill = P02_factor )) + 
  geom_bar(stat = 'identity', position = "fill") + 
  scale_fill_manual(values =c("#647A8F","#FFAC8E")) +
  labs(title = "2012", caption = "", fill = "Sexo") +
  xlab("Proporción") +
  ylab("Grupo de edad") +
  theme_classic()
,
ggplot( df_t_cocina_UT2019_plot3 , aes(x = media_ponderada_horas_cocina , y = rango_edad_10_UT2019 , fill = s1p2_factor )) + 
  geom_bar(stat = 'identity', position = "fill") + 
  scale_fill_manual(values =c("#42C2FF","#FBD6D2")) +
  labs(title = "2019", caption = "", fill = "Sexo") +
  xlab("Proporción") +
  ylab("Grupo de edad") +
  theme_classic()

,
ncol=2
,
top = textGrob("Proporción del tiempo en preparar alimentos, por grupo de edad, por sexo 2012 vs 2019 (horas)", gp=gpar(fontsize=14,font=1)))

```


En el 2012, considerando los grupos de edades y el sexo, podemos ver que en el grupo de edad de entre 20 y 29 años, las mujeres son las que mayor cantidad de horas dedican semanalmente en cocinar en comparación con el resto de grupos de edad. En este grupo de edad, 7.69 horas es el promedio de horas semanales dedicadas a la cocina por las mujeres y 0.6 horas es el tiempo promedio semanal dedicado a la cocina por hombres.

En el 2019, considerando los grupos de edades y el sexo, podemos ver que en el grupo de edad de entre 30 y 39 años, las mujeres son las que mayor cantidad de horas dedican semanalmente en cocinar en comparación con el resto de grupos de edad. En este grupo, 8.42 horas es el promedio de horas semanales dedicadas a la cocina por las mujeres y 1.66 horas es el tiempo promedio semanal dedicado a la cocina por hombres.




```{r echo=FALSE, fig.height=6 , fig.width=12}

grid.arrange(

ggplot( df_t_cocina_UT2012_plot4 , aes(x = media_ponderada_horas_cocina , y = reorder(provincia_UT2012, media_ponderada_horas_cocina) )) + 
  geom_bar(stat = 'identity', position = 'dodge', fill = "lightblue") + 
  labs(title = "2012", caption = "") +
  geom_text(aes(label = round(media_ponderada_horas_cocina, digits = 2)  ), color ="#1A374D", size = 4,position = position_dodge(0.9), hjust = 1.2) +
  xlab("Media ponderada de horas") +
  ylab("Provincia") +
  theme_classic()
,

ggplot( df_t_cocina_UT2019_plot4 , aes(x = media_ponderada_horas_cocina , y = reorder(provincia_UT2019, media_ponderada_horas_cocina) )) + 
  geom_bar(stat = 'identity', position = 'dodge', fill = "lightblue") + 
  labs(title = "2019", caption = "") +
  geom_text(aes(label = round(media_ponderada_horas_cocina, digits = 2)  ), color ="#1A374D", size = 4,position = position_dodge(0.9), hjust = 1.2) +
  xlab("Media ponderada de horas") +
  ylab("Provincia") +
  theme_classic()

,
ncol=2
,
top = textGrob("Tiempo semanal en cocinar o preparar alimentos, por provincia 2012 vs 2019 (horas)", gp=gpar(fontsize=14,font=1)))


```





```{r echo=FALSE, fig.height=6 , fig.width=12}

grid.arrange(

ggplot( df_t_cocina_UT2012_plot5 , aes(x = media_ponderada_horas_cocina , y = provincia_UT2012 , fill = P02_factor )) + 
  geom_bar(stat = 'identity', position = 'dodge') + 
  scale_fill_manual(values =c("#5B7DB1","#FFAC8E")) +
  labs(title = "2012", caption = "", fill = "Sexo") +
  geom_text(aes(label = round(media_ponderada_horas_cocina, digits = 2)  ), color ="black", size = 4,position = position_dodge(0.9), hjust = 0) +
  xlab("Media ponderada de horas") +
  ylab("Provincia") +
  theme_classic()
,
ggplot( df_t_cocina_UT2019_plot5 , aes(x = media_ponderada_horas_cocina , y = provincia_UT2019 , fill = s1p2_factor )) + 
  geom_bar(stat = 'identity', position = 'dodge') + 
  scale_fill_manual(values =c("#5B7DB1","#FFAC8E")) +
  labs(title = "2019", caption = "", fill = "Sexo") +
  geom_text(aes(label = round(media_ponderada_horas_cocina, digits = 2)  ), color ="black", size = 4,position = position_dodge(0.9), hjust = 0) +
  xlab("Media ponderada de horas") +
  ylab("Provincia") +
  theme_classic()

,
ncol=2
,
top = textGrob("Tiempo semanal en cocinar o preparar alimentos, por provincia, por sexo 2012 vs 2019 (horas)", gp=gpar(fontsize=14,font=1)))

```

Considerando las dos principales provincias del Ecuador, en el caso del Pichincha, las mujeres dedicaron en promedio 7.85 horas semanales en cocinar o preparar alimentos y los hombres 1.17 horas. Por otro lado, en Guayas, las mujeres dedicaron en promedio 8.67 horas semanales en cocinar o preparar alimentos y los hombres 0.73 horas.





```{r echo=FALSE, fig.height=6 , fig.width=12}

grid.arrange(

ggplot( df_t_cocina_UT2012_plot5 , aes(x = media_ponderada_horas_cocina , y = provincia_UT2012 , fill =  P02_factor  )) + 
  geom_bar(stat = 'identity', position = 'fill') +
  scale_fill_manual(values =c("#5B7DB1","#FFAC8E")) +
  labs(title = "2012", caption = "", fill = "Sexo") +
  xlab("Proporción") +
  ylab("Provincia") +
  theme_classic()
,
ggplot( df_t_cocina_UT2019_plot5 , aes(x = media_ponderada_horas_cocina , y = provincia_UT2019 , fill = s1p2_factor )) + 
  geom_bar(stat = 'identity', position = 'fill') + 
  scale_fill_manual(values =c("#42C2FF","#FFC7C7")) +
  #scale_fill_hue(direction = -1) +
  labs(title = "2019", caption = "", fill = "Sexo") +
  xlab("Proporción") +
  ylab("Provincia") +
  theme_classic()

,
ncol=2
,
top = textGrob("Proporción del tiempo en preparar alimentos, por provincia 2012 vs 2019", gp=gpar(fontsize=14,font=1)))

```



En el 2019 a comparación del 2012, podemos encontrar mayor variabilidad en las proporciones de horas dedicadas a la cocina por sexo en cada provincia. En la mayoría de provincias, ha aumentado el aporte del promedio de horas semanales de los hombres en comparación con el gráfico del 2012. Esto concuerda con el primer gráfico que evidenciaba el aumento del promedio de horas semanales dedicados a la cocina por parte de los hombres para el año 2019.

En el caso de Galápagos, para el año 2012, los habitantes de Galápagos no formaron parte de la muestra, por tal motivo no existen registros en el dataset de la provincia de Galápagos en ese año.






```{r echo=FALSE}
###############################################################
####                  Series de tiempo                     ####
###############################################################

tiempo <- c(2012, 2019)
h <- c(df_t_cocina_UT2012_plot1$media_ponderada_horas_cocina[1]	, df_t_cocina_UT2019_plot1$media_ponderada_horas_cocina[1]	)
m <- c(df_t_cocina_UT2012_plot1$media_ponderada_horas_cocina[2]	, df_t_cocina_UT2019_plot1$media_ponderada_horas_cocina[2]	)


ggplot( data.frame(tiempo,h,m)  , aes(x = tiempo) ) + 
  geom_line(aes(y = h, colour = 'Hombres')) + geom_point(aes(y = h)) + 
  geom_line(aes(y = m, colour = 'Mujeres')) + geom_point(aes(y = m)) + 
  scale_color_manual(values = c("#5B7DB1","#FFAC8E")) +
  labs(title = "Tiempo semanal en cocinar y preparar alimentos por sexo en 2012 vs 2019 (horas)", colour  = "Sexo") + 
  xlab("Año") +
  ylab("Promedio de horas") +
  ylim(0,8.5) +
  theme_light()
```

El gráfico anterior nos permite observar el cambio en el promedio de horas semanales dedicados a cocinar o preparar alimentos entre 2012 y 2019 para hombres y mujeres. Recordemos que, en el 2012, las mujeres dedicaron 8.34 horas semanales en cocinar o preparar alimentos mientras que en el 2019 esa cifra bajó a 6.43. Por otro lado, en el caso de los hombres, en el 2012 dedicaron 0.88 horas semanales en promedio mientras que el 2019 dedicaron 1.59 horas.

INTRODUCIR LOS RESULTADOS POR SERIE DE TIEMPO, PARA LUEGO PRESENTAR LOS OTROS. AL DE SERIE DE TIEMPO INCLUIR LAS DESVIACIONES (LA PONDERADA). SOLO EN LA SERIE DE TIEMPO. 

En nuestro GitHub, podrás revisar el código y reproducir el análisis.



__Referencias__

Instituto Nacional de Estadística y Censos INEC. (2013). "Metodología de la Encuesta Específica de Uso del Tiempo - 2012." Ecuador, Noviembre 2013. Recuperado de: https://www.ecuadorencifras.gob.ec//documentos/web-inec/Uso_Tiempo/Metodologia_EUT_2012.pdf

Instituto Nacional de Estadística y Censos INEC. (2012). "Encuesta específica de uso del tiempo EUT 2012" Ecuador, Noviembre 2012. Recuperado de: https://www.ecuadorencifras.gob.ec//documentos/web-inec/Uso_Tiempo/Formulario_2012.pdf

Instituto Nacional de Estadística y Censos INEC. (2012). "Encuesta específica de uso del tiempo EUT 2012" [base de datos]. Ecuador. Recuperado de: https://www.ecuadorencifras.gob.ec/uso-del-tiempo-2/#:~:text=Uso%20del%20Tiempo%20%E2%80%93%202012,y%20formulaci%C3%B3n%20de%20pol%C3%ADticas%20p%C3%BAblicas.

Instituto Nacional de Estadística y Censos INEC. (2020). "Documento metodológico de la Encuesta Nacional Multipropósito de Hogares (Seguimiento al Plan Nacional de Desarrollo)-2019" Ecuador, 2020. Recuperado de: https://www.ecuadorencifras.gob.ec/documentos/web-inec/Multiproposito/2019/201912_Metodologia_Multiproposito.pdf 

Instituto Nacional de Estadística y Censos INEC. (2019). "Encuesta Nacional Multipropósito de Hogares-Diciembre-2019" Recuperado de: https://www.ecuadorencifras.gob.ec/documentos/web-inec/Multiproposito/2019/201912_Formulario_Multiproposito.pdf


Instituto Nacional de Estadística y Censos INEC. (2020). "Guia para el usuario de base de datos. Encuesta Nacional Multipropósito de Hogares (Seguimiento al Plan Nacional de Desarrollo)" Ecuador, junio 2020. Recuperado de: https://www.ecuadorencifras.gob.ec/documentos/web-inec/Multiproposito/2019/201912_%20Guia%20de%20usuario%20BDD_Multiproposito.pdf 

Instituto Nacional de Estadística y Censos INEC. (2019). "Encuesta Nacional Multipropósito de Hogares (Seguimiento al Plan Nacional de Desarrollo)-2019" [base de datos]



https://www.rimisp.org/noticia/ecuador-las-mujeres-dedican-3-veces-mas-tiempo-al-trabajo-no-remunerado/


https://www.ecuadorencifras.gob.ec/el-inec-tambien-genera-estadisticas-de-trabajo-no-remunerado/
























