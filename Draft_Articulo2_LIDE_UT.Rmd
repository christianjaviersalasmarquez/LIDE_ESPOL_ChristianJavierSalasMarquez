...
En este artículo, vamos a calcular el promedio de horas semanales que los ecuatorianos dedican a hacer ejercicio o practicar deportes y ver cómo ha evolucionado este promedio comparando los resultados del año 2012 con los del 2020. Buscamos comparar los resultados de la pregunta: En la semana pasada... ¿Hizo ejercicios o practicó algún deporte?. Tanto en la encuesta de uso del tiempo del 2012 como en la encuesta nacional multipropósito del 2020 se encuentra esta pregunta.
...
Esta visualización nos permite descubrir que en todos los grupos de edad considerados, tanto en el 2012 como el 2020, los hombres dedican una mayor cantidad de horas semanales en hacer ejercicio o practicar deportes. 


```{r echo=FALSE, fig.height=6, fig.width=12}
grid.arrange(

ggplot( df_t_deporte_UT2012_plot4  %>% filter(provincia_UT2012 != 'Zonas no delimitadas') , aes(x = media_ponderada_horas_deporte , y = reorder(provincia_UT2012, media_ponderada_horas_deporte) )) + 
  geom_bar(stat = 'identity', position = 'dodge', fill = "lightblue") + 
  xlim(0,3) +
  labs(title = "2012", caption = "") +
  geom_text(aes(label = round(media_ponderada_horas_deporte, digits = 2)  ), color ="#1A374D", size = 4,position = position_dodge(0.9), hjust = 1.2) +
...
ggplot( df_t_deporte_UT2020_plot4 %>% 
       mutate(provincia_UT2020 = fct_relevel(provincia_UT2020, 
                                                ...
                                                'Zamora Chinchipe',
                                                'Galápagos')) %>% 
       filter(provincia_UT2020 != 'Galápagos') , aes(x = media_ponderada_horas_deporte , y = reorder(provincia_UT2020, media_ponderada_horas_deporte) )) + 
  geom_bar(stat = 'identity', position = 'dodge', fill = "lightblue") + 
  xlim(0,3) +
  labs(title = "2020", caption = "") +
  geom_text(aes(label = round(media_ponderada_horas_deporte, digits = 2)  ), color ="#1A374D", size = 4,position = position_dodge(0.9), hjust = 1.2) +
...
ggplot( df_t_deporte_UT2020_plot4 %>% 
       mutate(provincia_UT2020 = fct_relevel(provincia_UT2020, 
                                                ...
                                                'Galápagos')) %>% 
       filter(provincia_UT2020 != 'Galápagos') , aes(x = media_ponderada_horas_deporte , y = provincia_UT2020 )) + 
  geom_bar(stat = 'identity', position = 'dodge', fill = "lightblue") + 
  xlim(0,3) +
  labs(title = "2020", caption = "") +
  geom_text(aes(label = round(media_ponderada_horas_deporte, digits = 2)  ), color ="#1A374D", size = 4,position = position_dodge(0.9), hjust = 1.2) +
...
ggplot(df_t_deporte_UT2020_plot5 %>% 
       mutate(provincia_UT2020 = fct_relevel(provincia_UT2020, 
                                                ...
                                                #'Zonas no delimitadas',
                                                'Galápagos')) %>% 
       filter(provincia_UT2020 != 'Galápagos')  , aes(x = media_ponderada_horas_deporte , y = provincia_UT2020 , fill = s1p2_factor )) + 
  geom_bar(stat = 'identity', position = 'fill') + 
  scale_fill_manual(values =c("#647A8F","#FFAC8E")) +
  labs(title = "2020", caption = "", fill = "Sexo") +
  xlab("Proporción") +
...
En este caso, observamos que en todas las provincias, los hombres en promedio dedican una mayor cantidad de horas semanales en hacer ejercicio o practicar deporte que las mujeres. 
