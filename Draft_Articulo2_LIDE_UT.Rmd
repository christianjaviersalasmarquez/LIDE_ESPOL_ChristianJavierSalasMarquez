...
for (i in 1:R) {
  ...
  if((edad_2020[i] >= 0 ) & (edad_2020[i] <= 11)) { rango_edad[i] <- 'Edad entre 0 y 11 años' }
  else if ( (edad_2020[i] >=12  ) & (edad_2020[i] <= 19  ) ) { rango_edad[i] <- 'Edad entre 12 y 19 años' }
  else if ( (edad_2020[i] >=20  ) & (edad_2020[i] <= 24  ) ) { rango_edad[i] <- 'Edad entre 20 y 24 años' }
  else if ( (edad_2020[i] >=25  ) & (edad_2020[i] <= 29  ) ) { rango_edad[i] <- 'Edad entre 25 y 29 años' }
  else if ( (edad_2020[i] >=30  ) & (edad_2020[i] <= 34  ) ) { rango_edad[i] <- 'Edad entre 30 y 34 años' }
  else if ( (edad_2020[i] >=35  ) & (edad_2020[i] <= 39  ) ) { rango_edad[i] <- 'Edad entre 35 y 39 años' }
  else if ( (edad_2020[i] >=40  ) & (edad_2020[i] <= 44  ) ) { rango_edad[i] <- 'Edad entre 40 y 44 años' }
  else if ( (edad_2020[i] >=45  ) & (edad_2020[i] <= 49  ) ) { rango_edad[i] <- 'Edad entre 45 y 49 años' }
  else if ( (edad_2020[i] >=50  ) & (edad_2020[i] <= 54  ) ) { rango_edad[i] <- 'Edad entre 50 y 54 años' }
  else if ( (edad_2020[i] >=55  ) & (edad_2020[i] <= 59  ) ) { rango_edad[i] <- 'Edad entre 55 y 59 años' }
  else if ( (edad_2020[i] >=60  ) & (edad_2020[i] <= 64  ) ) { rango_edad[i] <- 'Edad entre 60 y 64 años' }
  else if ( (edad_2020[i] >=65  ) & (edad_2020[i] <= 69  ) ) { rango_edad[i] <- 'Edad entre 65 y 69 años' }
  else if ( (edad_2020[i] >=70  )) { rango_edad[i] <- 'Edad mayor a 70 años' }
  
  
}

df_analisis_UT2020$rango_edad <- factor(rango_edad)
...
```{r}
# En la semana pasada ¿Hizo ejercicios o practicó algún deporte? Por rango de edad
df_deporte_edad2020_5 <- df_analisis_UT2020 %>%
  group_by(rango_edad, s6p4_factor) %>%
  summarize(conteo_deporte = summary(factor(s6p4)))
...
```{r}
ggplot(df_deporte_edad2020_5 %>% filter(s6p4_factor == "Si" | s6p4_factor == "No"), aes(x = conteo_deporte  , y = rango_edad , fill = s6p4_factor )) + 
  geom_bar(stat = 'identity', position = 'dodge') + 
...
De los que respondieron que sí, cuanto tiempo...? Promedio de horas semanales deporte, por edad

Considerar los pesos 

```{r}
# VERIFICAR
df_analisis_UT2020_edad_summary_5 <- df_analisis_UT2020 %>%
  filter( s6p4_factor == "Si" ) %>%
  group_by(rango_edad) %>%
  summarize( promedio_horas_deporte = mean(horas_semanales_deporte), sd = sd(horas_semanales_deporte), suma = sum(horas_semanales_deporte), max = max(horas_semanales_deporte), min = min(horas_semanales_deporte) )

df_analisis_UT2020_edad_summary_5

```
Considerar por grupos de 10 años.
...
```{r}
ggplot(df_analisis_UT2020_edad_summary_5, aes(x = promedio_horas_deporte, y = rango_edad)) + 
  labs(title = "Promedio de horas semanales de actividad física por edad en 2020") +
  geom_segment(aes(xend =0, yend = rango_edad), size = 2, color = "lightblue") +
  geom_point(size = 8, color = "blue") + 
  geom_text(aes(label = round(promedio_horas_deporte, digits = 1.5)), color ="white", size = 3) +
  theme_classic() +
  theme(axis.line.y = element_blank(),
...
Considerar el tiempo de respuesta de los que responden NO, o sea 0 horas, lo cual también influye en el promedio.
